...
export const isDraw=(gameBoard,currentID,currentPlayer)=>{
    const board=[...gameBoard]
    board[currentID]=currentPlayer
...
for (let  check=0; check<moveChecks.length; check++){
  for(let i=0;i<moveChecks[check].max;i += moveChecks[check].step){
    ...
    switch(series){
        ...
        case "1110" :
        case "2220" : return i + moveChecks[check].indexes[3];
        case "1101" :
        case "2202" : return i + moveChecks[check].indexes[2];
        case "1011" :
        case "2022" : return i + moveChecks[check].indexes[1];
        case "0112" :
        case "0222" : return i + moveChecks[check].indexes[0];
        default:
    }
  }
}
